...
An image alone states no table. The five fields of its format block that a
table settles - the state block's bytes, where the table's header stands,
the row's bytes, `P` and `N` - read zero until a tool writes them, so an
image shipped uncombined states no table rather than the one it was built
from.

## Where they stand

Each tree holds the eight: the Java jar on its classpath, the Go
...
A tool takes the image the table asks for - the variant from the header,
and under DTX2 the unit at payload byte 2 and the copies flag at byte 3 -
...
disagree with the bytes. abi.md 1 lays the image out, abi.md 5 states the
combine, and abi.md 4 states the rules the packager holds a DTX2 table to
first.
