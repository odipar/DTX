...
and a caller reads it off the pointer `DTX_advance` leaves on the 68000,
one value a column at the stride (abi.md 2).

A reader that repeats the table reads row `RR` after row `R` minus one;
...
- SPEC.md defines the format and requirements.md what it has to do. R6
  bounds what a table may contain, and a reader reports a table that breaks
  R6 and does not read further (R6.5). No table here breaks it.
- A reader reads `R`, `C`, `RR` and the width from the header, and under
