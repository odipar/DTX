...
For each `NAME.dtx` under `tables/`, the bytes of `NAME.rows`: every row
the table holds, from row 0 to row `R` minus one, each row column 0 first,
each value at its own width, most significant byte first, and nothing
between the values or the rows. That is the row as DTX0 lays it out
(SPEC.md 2.1), and it is what `DTX_read` gives on the 68000 (abi.md 2).

A reader that repeats the table gives row `RR` after row `R` minus one;
the file holds one pass, rows 0 to `R` minus one, and nothing of the
repeat. Hold your reader's `RR` to the header's instead.
...
with the copy of ST4 this repository holds, at the unit and ring the row
...
- SPEC.md states the format and requirements.md what it has to do. R6
  bounds what a table may hold, and a reader given a table that breaks R6
  reports it and reads no further (R6.5). No table here breaks it.
...
- Under DTX2 a reader checks every data set's first long against
  ...
  reads `dtx2-copies` wrongly: the flag byte picks the build (R5.10).
...
DTX. Nothing here holds it to how fast, or to how it is called.
