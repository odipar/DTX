...
`W` is the table's and not a column's. Every value is 1, 2 or 4 bytes
and one table is one of the three (requirements.md, R6.3), so a column
...
next row, and has its place in the table: the first advance reaches
row 0, the next row 1, and the advance after row `R` minus one reaches row
...
A **reader** reads rows out of a table, and a **writer** puts them in. A
...
odd offset and a reader reads its values as bytes.
