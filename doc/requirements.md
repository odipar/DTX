...
- **R6.1** `R` is 1 to 2147483647. The field is four bytes, and a reader
  reads it as a signed long, so a count above that is out of bounds and no
  row number reads as the $FFFFFFFF an advance ends on (abi.md 2).
- **R6.2** `C` is 1 to 256.
- **R6.3** One width a table: every value of it is 1, 2 or 4 bytes, and
...
- **R6.5** Where a table breaks any of these, or R5.6, or its payload's
  data sets do not declare its `k` and ST4's format version 7, a reader
  reports it and does not read further. What it reports is
  SPEC.md's.
