    ...
    public static int Period(Header header, Packed given)
    {
        ...
        for (int p = columns; ; p++)
        {
                    ...
                    && budget >= 1 && budget <= 65535)
            {
                ...
                if (given.Replayed && loop < p)
                {
                    throw new ArgumentException($"a replayed loop of {loop} rows is under"
                            + $" the period of {p}: a refill meets one mark at most");
                }
                return p;
            }
        }
        throw new ArgumentException($"no period from C of {columns} up meets N of"
